/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProjectsWhereUniqueInput } from "./ProjectsWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ProjectsUpdateInput } from "./ProjectsUpdateInput";

@ArgsType()
class UpdateProjectsArgs {
  @ApiProperty({
    required: true,
    type: () => ProjectsWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ProjectsWhereUniqueInput)
  @Field(() => ProjectsWhereUniqueInput, { nullable: false })
  where!: ProjectsWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ProjectsUpdateInput,
  })
  @ValidateNested()
  @Type(() => ProjectsUpdateInput)
  @Field(() => ProjectsUpdateInput, { nullable: false })
  data!: ProjectsUpdateInput;
}

export { UpdateProjectsArgs as UpdateProjectsArgs };
